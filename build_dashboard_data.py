...
def _row_image_rel(row: dict) -> str:
    """Site-root path (leading /) so assets resolve on static hosts like Render."""
    raw = (row.get("URL") or "").strip()
    if not raw:
        return ""
    rel = (ROOT / "dual_pipeline_bundle" / raw).as_posix()
    rel = rel.replace(ROOT.as_posix() + "/", "").lstrip("/")
...
def _dedupe_auto_chunks(chunks: list) -> list:
    """CSV may contain multiple runs per chunk_id; keep the last row (latest run)."""
    by_cid: dict = {}
    for c in chunks:
        by_cid[c["chunk_id"]] = c
    return [by_cid[k] for k in sorted(by_cid.keys())]
...
def main() -> None:
    ...
    for pid in sorted(by_id.keys()):
        ...
        o["autoChunks"] = _dedupe_auto_chunks(o["autoChunks"])
